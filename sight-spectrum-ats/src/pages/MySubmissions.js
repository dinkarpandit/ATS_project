...
const isRowChecked = (SubmissionId) => {
  return selectedSubmissionIds.includes(SubmissionId);
};
  ...
  const getCandidateData = ()=> {
    ...
    if(demand_id){
      ...
      axiosPrivateCall.get(`/api/v1/submission/listUserCreatedSubmissions?skip=0&limit=15&sort_field=${fetchOptions.sort_field}&sort_type=${fetchOptions.sort_type}`).then(res=>{
       console.log(res.data);
       setCandidateList(res.data);
       setIsDataLoaded(true)
      }).catch(e=>{
  ...
  const fetchMoreData = () => {
   if (isUserSearching) {
      ...
      axiosPrivateCall
        .get('/api/v1/submission/listUserCreatedSubmissions', {
          params: {
            skip: fetchOptions.skip + fetchOptions.limit,
            limit: fetchOptions.limit,
            sort_field: fetchOptions.sort_field,
            sort_type: fetchOptions.sort_type,
          },
        })
        .then((res) => {
          const moreCandidates = res.data;
          setCandidateList((prevList) => [...prevList, ...moreCandidates]);
          if (moreCandidates.length < fetchOptions.limit || moreCandidates.length === 0) {
            setHasMore(false);
          }
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        {clientShowPopup && <ClientTrackerPopup
          ...
          bulkTracker = {selectedSubmissionIds}
        ...
        <div className={styles.nav_container}>
          ...
          <DefaultButton
            ...
            setCandidatData(candidateList.map((i) => i?._id));
           }}
             text="Client Tracker"
          />
