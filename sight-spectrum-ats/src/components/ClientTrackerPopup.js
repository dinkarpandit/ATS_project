...
export function ClientTrackerPopup(props) {
  ...
  console.log(props,"psp")
  let Id = props?.candidatData
  console.log(Id,"id")
  let bulkTracker = props?.bulkTracker
  ...
  useEffect(() => {
    axiosPrivateCall
      .get("/api/v1/client/getClientName")
      .then((res) => {
        setData(res?.data?.companyNames);
        console.log(res?.data?.companyNames,"yuyy");
      })
      .catch((err) => {});
  
    axiosPrivateCall
      .get(`/api/v1/submission/getSubmissionDetails?submission_id=${bulkTracker.join(',')}`)
      .then((res) => {
        console.log(res,)
  ...
  const downloadData = () => {
    setTimeout(() => {
      ...
      axiosPrivateCall
        .get(`/api/v1/client/downloadClientTracker?company_name=${selectedValue}&submission_id=${bulkTracker.join(',')}`, {
          responseType: 'blob',
        })
        .then(response => {
