...
const listDemands = {
    controller: async (req, res) => {
        ...
        const submissionCount = await Promise.all(demands.map(async (demand) => {
            const demand_id = demand._id;
            let submission = await submission_services.getSubmissionByDemand(demand_id);
            const submission_count = submission.length;
        ...
        res.respond(submissionCount, 200, 'Demands fetched sucessfully');
    }
}
