...
const getSelectedCandidates = {
  controller: async (req, res) => {
    try{
      let candidates = await candidate_services.getSelectedCandidates(req.query.candidate_id);
      res.respond(candidates, 200, 'Selected candidates fetched successfully');
    } catch (error){
...
const updateCandidateCommand = {
  controller: async (req, res) => {
    let update_obj = req.body;
    await candidate_services.updateCommandCandidate(update_obj)
    res.respond("Candidate updated successfully", 200, 'Candidate updated successfully.');
  }
}
...
const downloadMyCandidates = {
  controller: async (req, res) => {

    let random_prefix = crypto.randomBytes(20).toString('hex')
    let candidates = await candidate_services.getUserCreatedCandidate(req.auth.user_id,req.query)
    let excel_candidates = candidates.map(c => {
      let transformed = {
        candidate_id: c?._id,
        candidate_name: c?.first_name + ' ' + c?.last_name,
...
const getCandidateStarredDetails = {
  controller: async (req, res) => {
    let Candidate = await candidate_services.getAllStarredMyCandidates()
    res.respond(Candidate, 200, 'Candidate fetched sucessfully');
  }
}
...
const getUserCandidates = {
  controller: async (req, res) => {
    let demand = await candidate_services. getUserCreatedCandidate(req.auth.user_id,req.query)
    res.respond(demand, 200, 'Demands fetched sucessfully');
  }
}
...
module.exports = {
  ...
  downloadMyCandidates,
  getCandidateStarredDetails
}