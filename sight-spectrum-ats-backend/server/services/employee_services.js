...
class user_services {
  ...
  static async getSalesJobRole() {
    try {
      const employees = await fast_connection.models.employee.find({ is_deleted: false });
      const salesEmployees = employees.filter(employee => employee.role === "sales");
      
      const jobRolesSet = new Set(salesEmployees.map(employee => employee.job_role));
      
      const uniqueJobRoles = Array.from(jobRolesSet);
  
      // Transforming the array into objects with 'key' and 'text' properties
      const formattedJobRoles = uniqueJobRoles.map((role, index) => ({
        key: `role${index + 1}`,
        text: role
      }));
