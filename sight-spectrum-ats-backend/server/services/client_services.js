...
class client_services {
  ...
  static async getSubmissionDetailss(submission_ids) {
    try {
      const submission_id_array = submission_ids.split(','); // Split the string into an array of submission IDs
  
      return await fast_connection.models.submission
        .find({ is_deleted: false, _id: { $in: submission_id_array } })
        .populate([{ path: 'demand' }, { path: 'candidate' }, { path: 'submitted_by', select: '_id first_name last_name' }]);
    } catch (error) {
      throw error;
    }
  }
